...
const WORKSPACE_KEY = "workspace-directory";
...
export async function saveWorkspaceHandle(handle) {
  ...
  await txDone(db, "readwrite", (store) => store.put(handle, WORKSPACE_KEY));
  db.close();
}

export async function getWorkspaceHandle() {
  const db = await openFolioDb();
  const value = await txDone(db, "readonly", (store) => store.get(WORKSPACE_KEY));
  db.close();
  return value || null;
}
...
export async function clearWorkspaceHandle() {
  const db = await openFolioDb();
  await txDone(db, "readwrite", (store) => store.delete(WORKSPACE_KEY));
  db.close();
}
